<?php
/**
 * index.php
 *
 * Author: netman
 * DateTime:  2023/3/23 11:20
 */

use Zhaiwenjun\IcbcJft\Lib\IcbcConstants;
ini_set("display_errors", true);
require_once (dirname(__DIR__).'/vendor/autoload.php');

$config = [
    'aeskey' => '5xGJdh7qb+B95SUoxDlatg==',
    'app_id' => '10000000000000231071',
    'private_key' => '',
    'jft_public_key' => 'MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCwFgHD4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB',
];
$biz_content = [
    'appId'=>$config['app_id'],
    'outVendorId'=>'041910391',//子商户编号
    'outOrderId'=>'202312221313214122',
    'outUserId'=> '0202204300000000722',//用户标识
    'payAmount'=>10,
    'payType'=>'01',//支付类型 01:单笔支付，02：合并支付
    'varNote' => '订单备注',
    'notifyUrl' => '',
    'qrCode' => '',//用户付款码
    'goodsName' => '测试商品',
    'trxIp' => '121.21.12.52',
    'trxChannel' => '08',

];
$request = array(
    "serviceUrl" => 'https://apipcs3.dccnet.com.cn/ui/jft/ui/pay/h5/V3',
    "method" => 'POST',
    "isNeedEncrypt" => false,
    "extraParams" => null, //其他参数，用array
    "biz_content" => $biz_content
);



$sdk = new \Zhaiwenjun\IcbcJft\H5PayV3\SDK($config['app_id'],$config['private_key'],$config['jft_public_key'],$config['aeskey']);
$rtn = $sdk->execute($request);
$ss = "<html><head></head><body>".$rtn."</body></html>";
echo $ss;



